...
if sum_evens == sum_odds:
    modified = [str(el) for el in even_numbers.union(odd_numbers)]
    print(f"{', '.join(modified)}")

elif sum_odds > sum_evens:
    modified = [str(el) for el in odd_numbers.difference(even_numbers)]
    print(f"{', '.join(modified)}")

else:
    modified = [str(el) for el in even_numbers.symmetric_difference(odd_numbers)]
    print(f"{', '.join(modified)}")
